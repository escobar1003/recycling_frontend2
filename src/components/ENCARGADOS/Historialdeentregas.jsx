...
const navItems = [
  { id: "panel-control",       label: "Panel de control",   icon: "bi-speedometer2" },
  { id: "historial-entregas",  label: "Historial entregas", icon: "bi-clock-history" },
  { id: "historiar-entregas",  label: "Historiar entregas", icon: "bi-pencil-square" },
  { id: "canjes",              label: "Canjes",             icon: "bi-gift" },
  ...
  return (
    <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "16px 20px", flex: 1, minWidth: 130 }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 700, color: COLORS.text, display: "flex", alignItems: "center", gap: 8 }}>
        <i className={`bi ${icon}`} style={{ color: COLORS.green, fontSize: 18 }}></i>
        {value}
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ color: COLORS.textMuted, fontSize: 13 }}>Estado: </span>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "20px 24px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <i className="bi bi-recycle" style={{ fontSize: 40, color: COLORS.green, marginBottom: 8 }}></i>
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
            <button style={{ background: COLORS.greenLight, color: COLORS.green, border: `1px solid ${COLORS.green}`, borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              <i className="bi bi-pencil me-1"></i>Editar
            </button>
            <button style={{ background: "#fff7ed", color: "#c2410c", border: "1px solid #c2410c", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              Corregir pts
            </button>
          </div>
        </div>
              ...
              <tr key={d.id} style={{ borderBottom: `1px solid ${COLORS.border}`, background: i % 2 === 0 ? COLORS.white : "#fafdf9" }}>
                ...
                <td style={{ padding: "12px 16px" }}>
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <button style={{ background: "#fee2e2", color: "#991b1b", border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
            ...
            {historialManjes.map((r, i) => (
              <tr key={r.id} style={{ borderBottom: `1px solid ${COLORS.border}`, background: i % 2 === 0 ? COLORS.white : "#fafdf9" }}>
                ...
                <td style={{ padding: "12px 14px" }}>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button style={{ background: COLORS.greenLight, color: COLORS.green, border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                      <i className="bi bi-pencil me-1"></i>Editar
                    </button>
                    <button style={{ background: "#fee2e2", color: "#991b1b", border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                      <i className="bi bi-x"></i>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
        <StatCard label="Kg reciclados hoy"   value="150 kg" icon="bi-recycle" />
        <StatCard label="Usuarios atendidos"  value="45"     icon="bi-people-fill" />
        <StatCard label="Puntos otorgados"    value="12,500" icon="bi-star-fill" />
        <StatCard label="Canjes realizados"   value="12"     icon="bi-gift-fill" />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "20px 24px" }}>
          ...
          <div style={{ position: "relative", marginBottom: 14 }}>
            ...
            />
            <i className="bi bi-search" style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: COLORS.textMuted }}></i>
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <textarea
              ...
              style={{ width: "100%", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "8px 12px", fontSize: 14, background: COLORS.bg, minHeight: 56, resize: "vertical", boxSizing: "border-box" }}
            />
          </div>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "20px 24px", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <i className="bi bi-recycle" style={{ fontSize: 36, color: COLORS.green, marginBottom: 4 }}></i>
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "20px 24px" }}>
        <div style={{ fontWeight: 700, fontSize: 16, color: COLORS.text, marginBottom: 16 }}>Entregas recientes</div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: COLORS.bg }}>
              {["Usuario", "Material", "Peso", "Puntos", "Fecha", "Estado"].map((h) => (
          ...
          <tbody>
            {deliveries.slice(0, 4).map((d, i) => (
              <tr key={d.id} style={{ borderBottom: `1px solid ${COLORS.border}`, background: i % 2 === 0 ? COLORS.white : "#fafdf9" }}>
                ...
                <td style={{ padding: "10px 12px", color: COLORS.text }}>{d.peso}</td>
                <td style={{ padding: "10px 12px", color: COLORS.gold, fontWeight: 700 }}>{d.puntos}</td>
                <td style={{ padding: "10px 12px", color: COLORS.textMuted }}>{d.fecha}</td>
                <td style={{ padding: "10px 12px" }}><StatusBadge estado={d.estado} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function PageContent({ state, dispatch }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 300, color: COLORS.textMuted }}>
      <i className="bi bi-tools" style={{ fontSize: 48, marginBottom: 12 }}></i>
...
export default function Historialdeentregas() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", background: COLORS.bg, marginTop: -24, marginLeft: -24 }}>

      {/* Sidebar interno */}
      <aside style={{ width: 220, background: COLORS.sidebar, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "22px 20px 18px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, background: COLORS.greenMid, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <i className="bi bi-recycle" style={{ color: "#fff", fontSize: 18 }}></i>
            </div>
            <div>
        ...
        <nav style={{ flex: 1, padding: "12px 0" }}>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <i className={`bi ${item.icon}`} style={{ fontSize: 16, width: 18 }}></i>
                {item.label}
              </button>
            );
          })}
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: "50%", background: COLORS.greenMid, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <i className="bi bi-person-fill" style={{ color: "#fff", fontSize: 18 }}></i>
          </div>
          <div>
